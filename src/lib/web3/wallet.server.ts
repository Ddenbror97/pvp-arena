...
export type ChallengeResult =
  { ok: true; challengeId: string; message: string } | { ok: false; code: string };

export async function issueChallenge(userId: string, rawAddress: string): Promise<ChallengeResult> {
  if (!isValidEvmAddress(rawAddress) && !/^0x[0-9a-f]{40}$/.test(rawAddress))
  ...
  const { data, error } = await rpc("wallet_issue_challenge", {
    p_user: userId,
  ...
  if (!r.ok) {
    await log(rpc, "WALLET_VERIFICATION_FAILED", userId, {
      address: shortAddress(checksummed),
      reason: r.code,
    });
...
export async function verifyChallenge(
  ...
  const fail = async (code: string, addr?: string) => {
    await log(rpc, "WALLET_VERIFICATION_FAILED", userId, {
      address: addr ? shortAddress(addr) : null,
      reason: code,
    });
  ...
  if (!(await signatureMatches(message, signature, c.address)))
    return fail("INVALID_SIGNATURE", c.address);
  const { normalized } = normalizeAddress(c.address);
  const res = await rpc("wallet_consume_and_verify", {
    p_id: challengeId,
    p_user: userId,

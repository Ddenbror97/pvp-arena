...
export async function runWithdrawalWorker(chainId: number) {
  ...
  for (const w of rows ?? []) {
    if (w.status === "SUBMITTING" || w.status === "SUBMITTED") {
      inFlight = true;
      // Finality policy lives in ./payout-finality (safe block + two-provider agreement).
      const decision = await decidePayout({ primary: env.client as any, secondary: env.clientB as any, networkMode: env.networkMode }, w.tx_hash, safe);
      if (decision.action !== "no_receipt") {
        results[w.id] = await applyPayoutDecision((fn, args) => must(rpc(fn, args)), w.id, w.tx_hash, decision);
        continue;
      }

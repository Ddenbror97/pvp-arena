...
export function PlayerList({ players, pot, meId, winnerId }: { players: PlayerRow[]; pot: number; meId: string | null; winnerId?: string | null | undefined }) {
  ...
  if (!players.length) {
    return <p className="px-4 py-10 text-center text-sm text-muted-foreground">No players yet. Be the first in.</p>;
  }
  return (
      ...
      {sorted.map((p) => {
        ...
        return (
          <li
            key={p.user_id}
            className={cn(
              "flex items-center gap-3 px-4 py-3 transition-colors animate-rise-in",
              isMe && "bg-primary/5",
              winnerId === p.user_id && "bg-gold/10",
            )}
          >
            <span className="h-8 w-1 rounded-full" style={{ background: color }} />
            <PlayerAvatar src={p.profiles?.avatar_url} name={p.profiles?.username} className="h-9 w-9" color={color} />
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="truncate font-semibold">{p.profiles?.username ?? "player"}</span>

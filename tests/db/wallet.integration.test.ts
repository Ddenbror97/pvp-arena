...
const issue = async (u: string, a: string) =>
  (await sql`select pvp_test.wallet_issue_challenge(${u}::uuid, ${a}) r`)[0].r as Ch;
const consume = async (id: string, u: string, n: string) =>
  (await sql`select pvp_test.wallet_consume_and_verify(${id}::uuid, ${u}::uuid, ${n}) r`)[0].r as {
    ok: boolean;
    code?: string;
  };
const msg = (c: Ch) =>
...
d("wallet identity (isolated schema)", () => {
  ...
  it("issues a secure, DB-timed 10-minute challenge", async () => {
    ...
    const [{ drift }] =
      await sql`select abs(extract(epoch from (clock_timestamp() - ${c2.issued_at}::timestamptz))) drift`;
  ...
  it("rejects invalid input and unknown users", async () => {
    expect(
      (await issue(randomUUID(), privateKeyToAccount(generatePrivateKey()).address)).code,
  ...
  it("expired challenge is rejected by the database clock", async () => {
    ...
    expect(
      (
        await sql`select count(*)::int c from pvp_test.user_wallets where user_id=${u} and is_verified`
      )[0].c,
    ).toBe(0);
  });
  ...
  it("another user's challenge and a different address are rejected", async () => {
    ...
    expect(
      (await consume(c.id, u1, privateKeyToAccount(generatePrivateKey()).address.toLowerCase()))
        .code,
  ...
  it("20 concurrent verifications of one challenge: exactly one succeeds", async () => {
    ...
    const rs = await Promise.all(
      Array.from({ length: 20 }, () => consume(c.id, u, a.toLowerCase())),
    );
  ...
  it("one wallet can't be verified on two accounts, even racing in parallel", async () => {
    ...
    const rs = await Promise.all([
      consume(c1.id, u1, a.toLowerCase()),
      consume(c2.id, u2, a.toLowerCase()),
    ]);
    expect(rs.filter((r) => r.ok).length).toBe(1);
    ...
    const [row] =
      await sql`select consumed_at from pvp_test.wallet_verification_challenges where id=${loser.c.id}`;
  ...
  it("mixed-case and lowercase addresses are one wallet", async () => {
    ...
    expect(
      (await sql`select count(*)::int c from pvp_test.user_wallets where user_id=${u}`)[0].c,
    ).toBe(1);
    await expect(
      sql`insert into pvp_test.user_wallets (user_id, address, normalized_address) values (${u}, ${a}, ${a})`,
    ).rejects.toThrow();
  });
  ...
  it("an account switch never replaces the verified wallet", async () => {
    ...
    const rows =
      await sql`select normalized_address, is_verified, is_primary from pvp_test.user_wallets where user_id=${u} order by is_verified desc`;
    expect(rows).toHaveLength(2);
    expect(rows[0]).toMatchObject({
      normalized_address: a.toLowerCase(),
      is_verified: true,
      is_primary: true,
    });
  ...
  it("rate limits challenge creation and writes audit events", async () => {
    ...
    expect(
      (
        await sql`select count(*)::int c from pvp_test.auth_events where user_id=${u} and event='WALLET_VERIFIED'`
      )[0].c,
    ).toBe(1);
    await expect(
      sql`select pvp_test.wallet_log('ANYTHING', ${u}::uuid, '{}'::jsonb)`,
  ...
  it("test-credit balances are untouched by wallet actions", async () => {
    const u = await newUser(uname());
    const before =
      await sql`select kind, balance from pvp_test.wallet_accounts where owner_id=${u} order by kind`;
    const a = privateKeyToAccount(generatePrivateKey()).address;
    const c = await issue(u, a);
